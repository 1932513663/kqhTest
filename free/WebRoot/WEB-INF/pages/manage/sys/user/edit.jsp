...
$(function(){
	...
	selectDept(companyId,deptId);
	selectPost(companyId,deptId,postId);
});

//选择公司
function selectCompany(selectId){
	$('#companyName').combotree({    
	    ...
	    onLoadSuccess:function(node, data){
	    	if(selectId){
	    		$('#companyName').combotree('setValue', selectId);
	    	}
	    }
	}); 
}
//选择部门
function selectDept(companyId,selectId){
	$('#deptName').combotree({    
	    ...
	    onLoadSuccess:function(node, data){
	    	if(selectId){
	    		$('#deptName').combotree('setValue', selectId);
	    	}
	    }
	}); 
}
//选择职位
function selectPost(companyId,deptId,selectId){
	$('#postName').combotree({    
	    ...
	    onLoadSuccess:function(node, data){
	    	if(selectId){
	    		$('#postName').combotree('setValue', selectId);
	    	}
	    }
	}); 
}
		...
		<form id="user_form">
			<table>
				...
				<tr>
					...
					<td><input id="userName" name="userName" class="easyui-textbox" data-options="required: true" style="width:200px;" value="${user.userName }"></td>
				</tr>
			</table>
			...
			<input id="postId" name="postId" type="hidden" value="${user.postId }"/>
		</form>
	</div>
